  ...
  for (let i = 0; i < addressVariations.length; i++) {
    ...
    try {
      ...
      const response = await fetch(url, {
        headers: {
          'User-Agent': 'YEFF-Alert-Section2-Monitor/1.0'
        }
      });
...
async function processSection2() {
  try {
    console.log('🔄 Section 2: Processing stations 1701-2634');
    ...
    // 1701-2634 범위 추출 (인덱스 1700-2633)
    const sectionData = rawData.slice(1700, 2634);
    console.log(`📊 Section 2 data: ${sectionData.length} stations (1701-2634)`);
    ...
    for (let i = 0; i < sectionData.length; i++) {
      ...
      if (geocodeResult) {
        ...
        station.processed_section = 'section2';
        successCount++;
      ...
      // 50개마다 진행 상황 출력
      if ((i + 1) % 50 === 0) {
        console.log(`\n📊 Section 2 Progress: ${i + 1}/${sectionData.length}`);
        console.log(`Success: ${successCount}, Failed: ${failCount}`);
        console.log(`Success rate: ${((successCount / (i + 1)) * 100).toFixed(1)}%\n`);
        
        // 중간 저장
        const partialPath = path.join(__dirname, '..', 'public', 'data', `section2_partial_${i + 1}.json`);
    ...
    // 최종 결과 저장
    const outputPath = path.join(__dirname, '..', 'public', 'data', 'section2_complete_1701_2634.json');
    ...
    // 백업 파일 생성
    const backupPath = path.join(__dirname, '..', 'public', 'data', `section2_backup_${Date.now()}.json`);
    fs.writeFileSync(backupPath, JSON.stringify(pollingStations, null, 2));
    console.log(`💾 Backup saved: ${backupPath}`);
    
  } catch (error) {
    console.error('❌ Error in section 2 processing:', error);
  }
}

console.log('🚀 Starting Section 2 processing (1701-2634)...');
